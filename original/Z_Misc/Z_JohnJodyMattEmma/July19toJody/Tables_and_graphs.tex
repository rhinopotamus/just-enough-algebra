...
To make sense of these numbers, suppose there are five friends.  Three don't smoke at all, so that is 0 cigarettes in a year.  Another smokes only occasionally, maybe 100 cigarettes a year.  The fifth smokes ``a pack a day,''  which adds up to 7,300 cigarettes in a year because
...
%You'll get the hang of this guess method, but here's something you can try if guessing isn't working.  Take the maximum value you want on your graph (say 4,500), divide by the number of gridlines available.  Suppose we had up to 16 boxes.  Then
%$$4,500 \div 16 \text{ boxes} = 281.25 \text{ per box}$$ That gives you the smallest number each box needs to count.  Now round up to a nice whole number (to make it easier to plot points):  375 rounds up to 500.  There it is.
...
%\end{enumerate}
%
%In 1930 we have $Y=30$ and $S=1,485$.  That $S$-value is almost equal to 1,500.  We draw the point just a tiny bit below the gridline for 1,500.  It's okay if the point you draw looks on that line, but officially it's a tiny bit below. 
